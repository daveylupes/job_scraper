...
def parse_h2_structure(soup, url, job_title):
    ...
    for job in job_listings:
        ...
        if job_anchor and job_title.lower() in job_anchor.text.lower():
            ...
            if info_bar:
                category_location = info_bar.find(
                    "div", class_="category-location"
                ).text.strip()
...
def parse_tr_structure(soup, url, job_title):
    ...
    for job in job_listings:
        ...
        if job_anchor and job_title.lower() in job_anchor.text.lower():
            ...
            category = (
                job.find("td", class_="job-search-results-category")
                .find("li")
                .text.strip()
            )
            ...
            workplace_type = job.find(
                "td", class_="job-search-results-workplace-types"
            ).text.strip()
